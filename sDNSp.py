...
import sys, time, datetime, random, threading, traceback, struct, json
import socketserver, pycurl
...
def myIP():
    ...
    if Ext_eip: # 扩展IP探测
        ...
        for i,ip in enumerate(ipl):
            ...
            c.setopt(c.WRITEFUNCTION, wdf)
            try: c.perform();   # print('cc = %s\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'%c.getinfo(c.PRIMARY_IP))
            except pycurl.error as err: cprint("[Error]:[%s] %s" % (ip, err), 0xE); # qip[ip]=err
            else: qip[ip]=wr_buf[0],wr_buf[8][1:-1]; fb = True; p.append(ip); # print(qip[ip])
            time.sleep(.3)
            c.close()
            del c, buf
    for i in qip:
...
'''# Curl obj list init  ---------------------------------------------------------------------
 def cinit:
    c = []; f = []
    for ip in ipl:  # i,ip in enumerate(ipl):
        f.append(True)
        n = pycurl.Curl()
        n.setopt(n.NOPROGRESS, 1)
        n.setopt(n.CONNECTTIMEOUT_MS, 600)
        n.setopt(n.TIMEOUT_MS, 1200)
        n.setopt(n.MAXREDIRS, 0)
        n.setopt(n.USERAGENT, 'Curl')
        n.setopt(n.TCP_KEEPALIVE, 1)
        n.setopt(n.TCP_KEEPIDLE, 300)
        n.setopt(n.TCP_KEEPINTVL, 60)
        n.setopt(n.DEFAULT_PROTOCOL, 'https')
        n.setopt(n.SSLVERSION, n.SSLVERSION_TLSv1_2)
        n.setopt(n.SSL_VERIFYPEER, 0)
        n.setopt(n.SSL_VERIFYHOST, 0)
        n.setopt(n.SSL_ENABLE_NPN, 0) 
        n.setopt(n.SSL_ENABLE_ALPN, 0)
        n.setopt(n.RESOLVE, ['dns.google.com:443:'+ip])
        c.append(n)
        del n'''
...
def cget(qn='g.cn', eip=''):
        ...
        if f[i]:    # 加个标记锁，防止多个线程同时调用一个Curl对象时出错 "cannot invoke setopt() - perform() is currently running"
            ...
            for n in range(2):  # 超时后重试
                try: c[i].perform()
                    ...
                    dil[ipl[i]] += 1    # IP超时计数
                    # cprint("[Error-%d]:[%s] %s" % (n+1,ipl[i], err), 0xE)
                else:
    ...
    '''for i,ip in enumerate(ipl): # 顺序使用IP列表，越靠前的IP利用率越高
        if f[i]:    # 加个标记锁，防止多个线程同时调用一个Curl对象时出错 "cannot invoke setopt() - perform() is currently running"
            f[i] = False
            c[i].setopt(c[i].WRITEDATA, buf)
            c[i].setopt(c[i].URL, 'https://dns.google.com/resolve?name=' + qn + eip)
            for n in range(2):  # 超时后重试
                try: c[i].perform()
                    ...
                    dil[ip] += 1    # IP超时计数
                    cprint("[Error-%d]:[%s] %s" % (n+1,ip, err), 0xE)
                else:
                    cprint('[%s]: 新建连接: %d' % (c[i].getinfo(c[i].PRIMARY_IP), c[i].getinfo(c[i].NUM_CONNECTS)),2)
                    rsp = buf.getvalue()
                    if len(rsp) > 8 and rsp[:9] == b'{"Status"': f[i] = True; sil[ip] += 1; return rsp # 检查数据并返回
                    else: cprint('[Error]: -----rsp data Error----:\tip==[%s]\n%s\n'%(ip,rsp),14); break
            f[i] = True
            if dil[ip] > 10: print('---Connect to [%s] Err:%d|OK:%d'%(ip,dil[ip],sil[ip]))
            time.sleep(.1)'''
    del buf, il
...
class BaseRequestHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        try:
            data = self.get_data()
            # print('len=%d: %s'%(len(data),''.join('%02X '%x for x in data)), end='\n')  # repr(data).replace('\\x', '')[1:-1]
...
class TCPRequestHandler(BaseRequestHandler):
    ...
    def get_data(self):
        ...
        # int.from_bytes(data[:2],'big')   int(binascii.hexlify(data[:2]),16)   int.from_bytes([255,255,255,255],'big')   codecs.encode(b'\x12\xcd','hex')
        if sz < len(data) - 2:
